...
const BookCard = ({ book, onPreview }: BookCardProps) => {
	...
	return (
		<button
			...
			className="card-surface card-hover flex h-full w-full flex-col gap-4 overflow-hidden p-5 text-left animate-float-in transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(44,95,79,0.35)]"
			aria-label={`Preview details for ${book.title}`}
		>
			<div className="relative flex items-start gap-4">
				<div className="h-36 w-28 flex-shrink-0 overflow-hidden rounded-xl border border-shadow-light bg-cream">
					<img
